...
from .forms import CustomerUserForm, CustomerForm, ProductForm, FeedbackForm, ContactusForm, OrderForm
from .models import Feedback, Product, Orders
...
def update_customer_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            customerForm.save()
            return redirect('view-customer')
    return render(request, 'admin_updatecustomer.html', context=mydict)
...
def update_product_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if productForm.is_valid():
            productForm.save()
            return redirect('admin-products')
    return render(request, 'admin_updateproduct.html', {'productForm':productForm})
...
def admin_view_booking_view(request):
    ...
    for order in orders:
        ...
        ordered_products.append(ordered_product)
        ordered_bys.append(ordered_by)
    return render(request, 'admin_viewbooking.html', {'data':zip(ordered_products, ordered_bys, orders)})
...
def send_feedback_view(request):
    if request.method == 'POST':
        ...
        data = Feedback(name=name, message='message', feedback='feedback')
        data.save()
...
# actually we have to check whether payment is successful or not
...
def my_order_view(request):
    ...
    return render(request, 'myorder.html', {'data':zip(ordered_products, orders)})
...
def download_invoice_view(request,orderID,productID):
    ...
    return render_to_pdf('fashion/download_invoice.html',mydict)
...
def my_profile_view(request):
    customer=models.Customer.objects.get(user_id=request.user.id)
    return render(request, 'myprofile.html', {'customer':customer})
...
def edit_profile_view(request):
    ...
    return render(request, 'editprofile.html', context=mydict)
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(
                name,  # subject,
                message + 'email by' +' ' + email, # message,
                email,  # from email,
                ['arerirodney@gmail.com' ],  # to email
            )
